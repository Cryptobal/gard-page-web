...
- Eventos implementados (dataLayer): `pageview`, `submit_form_submission` (helper `trackFormSubmission`; conversión canónica GA4/Ads), `submit_form_reclutamiento` (postulaciones, no Ads), `cotizacion_submit`, `cotizar_exposure`, `cotiz_step_completed`, `cotiz_step_exposure`, `cotiz_submit`, `cotiz_partial_lead_detected/resumed/dismissed`, `recruit_cta_click`, `exit_intent_shown/dismissed/converted`, `scroll_depth`, `time_on_page`; tipos para `click_whatsapp`, `click_phone`, `click_email`, `click_cta_*`, `view_categoria_*` en `app/components/EventTracker.tsx`.
